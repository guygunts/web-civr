...
function View(Request $request)
{
 ...
 // PrintR($params);
	//exit;
    $url = URL_API . '/geniespeech/listpackget';
	...
	$drowdownspeed=$response['drowdownspeed'];
	 foreach ((array)$datalist as $i => $item) {
            ...
            $dataattr[$i] = $item;
           // if ($permiss[2]) {
		    $action .= '<button  class="btn btn-xs btn-primary " data-target="#Modal"  data-toggle="modal" onclick=getdata("'.$item['PACK_ID'].'")>Edit</button><button  class="btn btn-xs btn-danger " onclick=deletedata("'.$item['PACK_ID'].'")>Delete</button>';
           // } 
...
function getdata (Request $request){
	global $token;
    parse_str($request->getpost()->toString(), $data);
    $url = URL_API .'/geniespeech/getdatalistpackget';
...
function Add(Request $request)
{
    ...
    $url = URL_API . '/geniespeech/addlistpackget';
...
function Edit(Request $request)
{
    ...
    $data['user_login'] = $user;
    $url = URL_API . '/geniespeech/editlistpackget';
    $response = curlposttoken($url, $data, $token);
...
function Del(Request $request)
{
    ...
    parse_str($request->getPost()->toString(), $data);
    $url = URL_API . '/geniespeech/deletelistpackget';
    $response = curlposttoken($url, $data, $token);
...
function genxml(Request $request){
	global $token;
	parse_str($request->getPost()->toString(),$data);
	$url = URL_API . '/geniespeech/generatexml';
    $response = curlposttoken($url, $data, $token);
	 if ($response['code'] == 200) {
        $result['success'] = 'COMPLETE';
    } else {
