...
me.action.menu = 'service';
me.action.add = 'addservice';
me.action.edit = 'updateservice';
me.action.del = 'deleteservice';
...
me.Dele = function (e) {
    ...
    let attr =[]
        for(let i=0; i<e.length; i++){
            let data={
                "service_id":e[i].service_id
            }
            attr.push(e[i].DT_RowId)
            service_del.push(data)
        }
alertify.confirm("Do you want Delete.",
    function () {
        $.ajax({
            ...
            dataType: 'json',
            cache: false,
            data: {'menu_action' : me.action.del , 'service_del' : service_del },
            success: function (data) {
                switch (data.success) {
                    case 'COMPLETE':
                        $('.modal').modal('hide');
