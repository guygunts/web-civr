...
function View(Request $request)
{
    ...
    $params = array(
		...
		'enable' => $data['enable'],
		'Package_Sub_Code' => $data['Package_Sub_Code']
    );
    //PrintR($params);

    $url = URL_API . '/geniespeech/Listontopprepaid';
...
function Add(Request $request)
{
    ...
    $url = URL_API . '/geniespeech/insertontopprepaid';
...
function Edit(Request $request)
{
    ...
    $data['user_login'] = $user;
    $url = URL_API . '/geniespeech/updateontopprepaid';
    $response = curlposttoken($url, $data, $token);
...
function Del(Request $request)
{
    ...
    parse_str($request->getPost()->toString(), $data);
    $url = URL_API . '/geniespeech/deleteontopprepaid';
    $response = curlposttoken($url, $data, $token);
...
function genxml(Request $request){
	global $token;
	parse_str($request->getPost()->toString(),$data);
	$url = URL_API . '/geniespeech/generatexmlprepaid';
    $response = curlposttoken($url, $data, $token);
	 if ($response['code'] == 200) {
        $result['success'] = 'COMPLETE';
    } else {
