...
$('form#frmsearch').submit(function () {
		...
		let Package_Sub_Code_search=document.getElementById('Package_Sub_Code_search').value
		me.table.clear().draw();
		me.LoadDataontopprepaid(me.action.menu, 1, page_size,group,enable,text_search,Package_Sub_Code_search)
...
me.add= function (){
	...
	if(datajson.Pack_file_info.search(".wav") >=0 || datajson.Pack_file_info ==''){
...
//document.getElementById("START_DATE").value=newDate + " " + newTime
	//document.getElementById("END_DATE").value='12/31/'+objDate.getFullYear()+' '+'12:59:59 PM' objDate.toLocaleTimeString('en-GB')
...
function changedata(){
	var page_size = $('#page_size').val();
		...
		let Package_Sub_Code_search=document.getElementById('Package_Sub_Code_search').value
				me.table.clear().draw();
				me.LoadDataontopprepaid(me.action.menu,1, page_size,group,enable,text_search,Package_Sub_Code_search,1)
			//	me.table.page.len(page_size).draw();
}
me.LoadDataontopprepaid = function(menu, page_id, page_size,group,enable,search = '',Package_Sub_Code_search,readd){
	...
	$.ajax({
		...
		dataType:'json',
		cache:false,
		data:{ menu_action : menu , page_id : page_id , page_size : page_size ,packagegroup:group,enable:enable, text_search : search,Package_Sub_Code:Package_Sub_Code_search},
		success:function(data){
